...
const supabase = createClient<Database>(
  "https://ythvuinhhtbmtbnfgmol.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl0aHZ1aW5oaHRibXRibmZnbW9sIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Mzc1MjY0MTksImV4cCI6MjA1MzEwMjQxOX0.c7TXJw3n1up0iYwQCXKEIjafdQzwtGeD5pOMfwzFbws"
);
...
export async function createEmptyContact() {
  const { data, error } = await supabase.from('people').upsert({ id: "new"}).select()
  return data![0];
}
...
export async function getContact(id: string) {
  const { data, error } = await supabase.from('people').select('id,first,last,twitter,avatar,notes,favorite').eq('id', id);
  return data![0];
}
...
export async function updateContact(id: string, updates: ContactMutation) {
  const {data, error} = await supabase.from('people').update(
    {
      id: updates.first?`${updates.first!.toLocaleLowerCase()}-${updates.last!.toLocaleLowerCase()}`:id,
     ...updates 
    }).eq('id', id).select();
